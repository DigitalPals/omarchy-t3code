...
3. Run `pnpm install --frozen-lockfile` and `pnpm package` on x86-64 Linux, then
   run `pnpm bundle:marketplace` to refresh the root compressed runtime. Copy
   the newly generated `dist/plugin/licenses` inventory to root `licenses`,
   inspect both changes, and rerun `pnpm check` plus `pnpm package`.
4. Verify the root contains exactly one `manifest.json`, no tracked symlinks,
   ...
   inventory, and a runtime whose `bin/t3-mini-bridge --self-test` reports the
   release version and pinned T3 commit. Validate a clean Git checkout with
   `omarchy plugin validate` and test `omarchy plugin add`, update, login,
