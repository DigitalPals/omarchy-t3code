...
test("lifecycle, turns, approvals, and input map to real orchestration commands", async () => {
  const { commands, dispatched } = harness();
  await commands.send({ threadId: "thread-1", text: "Continue" });
  await commands.interrupt({ threadId: "thread-1" });
  await commands.settle({ threadId: "thread-1" });
  await commands.unsettle({ threadId: "thread-1" });
  await commands.snooze({ threadId: "thread-1", until: "2026-08-23T00:00:00.000Z" });
  await commands.unsnooze({ threadId: "thread-1" });
  await commands.pin({ threadId: "thread-1" });
  await commands.unpin({ threadId: "thread-1" });
  await commands.rename({ threadId: "thread-1", title: "Renamed" });
  await commands.regenerateTitle({ threadId: "thread-1" });
  await commands.respondApproval({ threadId: "thread-1", requestId: "approval-1", decision: "acceptForSession" });
  await commands.respondInput({ threadId: "thread-1", requestId: "input-1", answers: { choice: "Yes" } });
...
test("unsupported server capabilities reject lifecycle mutation locally", async () => {
  ...
  await assert.rejects(commands.settle({ threadId: "thread-1" }), (error: unknown) => (error as { code?: string }).code === "CAPABILITY_UNSUPPORTED");
  await assert.rejects(commands.snooze({ threadId: "thread-1", until: "2026-08-23T00:00:00.000Z" }), (error: unknown) => (error as { code?: string }).code === "CAPABILITY_UNSUPPORTED");
  await assert.rejects(commands.pin({ threadId: "thread-1" }), (error: unknown) => (error as { code?: string }).code === "CAPABILITY_UNSUPPORTED");
  await assert.rejects(commands.regenerateTitle({ threadId: "thread-1" }), (error: unknown) => (error as { code?: string }).code === "CAPABILITY_UNSUPPORTED");
});

test("model option changes are capability-validated and persisted with the full selection", async () => {
  const { commands, dispatched } = harness();
  await commands.setModelOption({ threadId: "thread-1", optionId: "reasoningEffort", value: "xhigh" });
  assert.deepEqual(dispatched[0]?.modelSelection, {
    instanceId: "codex",
    model: "gpt-5.6",
    options: [
  ...
  await assert.rejects(
    commands.setModelOption({ threadId: "thread-1", optionId: "reasoningEffort", value: "unsupported" }),
    (error: unknown) => (error as { code?: string }).code === "MODEL_OPTION_INVALID",
  );
});
...
test("screenshot-only turns dispatch actual pinned upload attachments and consume their staging ids", async () => {
  ...
  const draft = await commands.pasteClipboardImage({ threadId: "thread-1" });

  await commands.send({ threadId: "thread-1", text: "", attachmentIds: [draft.id] });
