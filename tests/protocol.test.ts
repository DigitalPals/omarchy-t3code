...
test("protocol decoder validates envelopes and operation payloads", () => {
  ...
  const option = decodeRequestLine(JSON.stringify({
    ...
    payload: { threadId: "thread-1", optionId: "reasoningEffort", value: "xhigh" },
  }));
  ...
  const imageOnly = decodeRequestLine(JSON.stringify({
    ...
    type: "thread.send",
    payload: { threadId: "thread-1", text: "", attachmentIds: ["attachment-1"] },
  }));
  ...
  assert.throws(() => decodeRequestLine(JSON.stringify({ protocolVersion: 1, requestId: "x", type: "thread.create", payload: { projectId: "p", prompt: "go", runtimeMode: "unsafe" } })), /runtimeMode/u);
  assert.throws(() => decodeRequestLine(JSON.stringify({ protocolVersion: 1, requestId: "x", type: "thread.send", payload: { threadId: "t", text: "", attachmentIds: [] } })), /message content/u);
  assert.throws(() => decodeRequestLine(JSON.stringify({ protocolVersion: 1, requestId: "x", type: "thread.send", payload: { threadId: "t", text: "go", attachmentIds: ["same", "same"] } })), /duplicates/u);
  assert.throws(() => decodeRequestLine(JSON.stringify({ protocolVersion: 1, requestId: "x", type: "attachment.discard", payload: { threadId: "t" } })), /attachmentId/u);
