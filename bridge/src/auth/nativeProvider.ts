  ...
  async login(): Promise<AuthStatusDto> {
    ...
    try {
      ...
      const completed = await this.clerkRequest(
        `/v1/client/sign_ins/${encodeURIComponent(signInId)}?rotating_token_nonce=${encodeURIComponent(completedCallback.rotatingTokenNonce)}`,
        clientToken,
        { method: "GET" },
        "T3 Connect browser callback",
      );
      clientToken = await this.rememberClientToken(completed.clientToken);
      const client = completed.client ?? (await this.currentClient(true))?.data ?? null;
